...
class Unionpay {
    ...
    */
    constructor(opts) {
        ...
        this.bizScene = opts.bizScene;
        this.payeeAcctNo = opts.payeeAcctNo;
        this.payeeAcctNm = opts.payeeAcctNm;
        this.payeeBankName = opts.payeeBankName;
        this.certId = '';
        this.publicKey = '';
        this.privateKey = '';
    }
    ...
    async initKey() {
        ...
        this.publicKey = fs.readFileSync(this.cert).toString();
    }
    ...
    async getFrontTn(obj) {
        ...
        return new Promise(async (resolve, reject) => {
            let formData = await utils.buildParams(obj, this);
            let frontTransUrl = this.sandbox ? gateway.frontTransReq.test : gateway.frontTransReq.product;
            const html = await utils.buildAutoFormHtml(formData, frontTransUrl);
     ...
     * @return tn 交易流水号 出错为null
     */
    async getBizFrontTn(obj) {
        // App支付特定类型设置
        obj.bizType = "000202";
        obj.txnType = "01";           // 交易类型
        obj.txnSubType = "01";        // 交易子类
        return new Promise(async (resolve, reject) => {
            let formData = await utils.buildBizParams(obj, this);
            let frontTransUrl = this.sandbox ? gateway.frontTransReq.test : gateway.frontTransReq.product;
            const html = await utils.buildAutoFormHtml(formData, frontTransUrl);
